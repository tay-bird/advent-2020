...
INPUT_FILE = 'input'
INPUT_MATCH = re.compile(r'(\d+)\-(\d+) (\w): (\S+)')


def is_compliant(entry):
    first_position, second_position, key_char, password = INPUT_MATCH.findall(entry)[0]
    print(entry)
    print(password[int(first_position)-1])

    return (password[int(first_position)-1] == key_char) ^ (password[int(second_position)-1] == key_char)  # XOR


def main():
    input = open(INPUT_FILE).read().splitlines()
    compliant_passwords = [password_entry for password_entry in input if is_compliant(password_entry) is True]

    print(len(compliant_passwords))
